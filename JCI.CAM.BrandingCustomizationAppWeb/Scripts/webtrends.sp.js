...
SharePoint 2013 Plugin v3.0.23
*/
(function () {
    WebTrendsSP_intra = function (tag, plugin) {
        this.enabled = true;
        this.tagObj = tag;
        this.tagVersion = "3.0.23";
...
var sp_intra_loader = function (tag, plugin) {
    ...
    Webtrends.addTransform(function (dcsObject, trackObject) {
            ...
            if (typeof (wt_sp_user) != "undefined") {
                try {
                    for (var prop in wt_sp_user) {
                        trackObject.argsa.push(prop, wt_sp_user[prop]);
                    }
                }
                catch (ex) {
                    if (cfg.debugmode) {
        ...
        if (cfg.bread) {
            try {
                var bcFinal = "";
                var titleSpan = document.getElementById("DeltaPlaceHolderPageTitleInTitleArea");
                if (titleSpan.getElementsByTagName("span").length == 0) {
                    bcFinal = titleSpan["innerText"].replace(/\s+/g, "") + ":";
                }
                else {
                    var nodes = titleSpan.getElementsByTagName("span")[0].getElementsByTagName("span");
                    if (nodes) {
                        for (var i = 0; i < nodes.length; i++) {
                            if (nodes[i]["innerText"] && nodes[i]["innerText"].replace(/\s+/g, "") != "") {
                                bcFinal = bcFinal + nodes[i]["innerText"] + ":";
                            }
                        }
                    }
                }
    ...
    if (_sp_intra.cfg.search) {
        ...
        try {
            if (typeof ($getClientControl) == "function") {

                //Add a method to send a multiTrack call when search results are ready
                var search = $getClientControl(document.getElementById(ids.searchBox));
                ...
                //Its possible that search results are ready before this executes.
                //If they are, send a multiTrack event.
                var results = $getClientControl(document.getElementById(ids.searchResult));
                if (results) {
                    var dataProvider = results.$2f_3;
                    var searchTerm = dataProvider.$2_3.k;
                    var resultCount = dataProvider.$9_3;
    ...
    if (_sp_intra.cfg.documentMenuClick) {
		...
		*/
        ...
        window.setTimeout(function () {
            ...
            if (typeof (ShowMenuForTrOuter) != 'undefined') {
